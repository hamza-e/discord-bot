...
import commands.defaut.GameTests;
...
public final class CommandMap {
	...
	public CommandMap(Discord bot) {
		this.bot=bot;
		registerCommand(new CommandDefault(bot));
		//registerCommand(new GameTests(bot));
	}
    ...
    public void commandConsole(String command){
        ...
        if(object[0] == null || ((SimpleCommand)object[0]).getExecuterType() == ExecuterType.USER){
    ...
    public boolean commandUser(User user, String command, Message message){
        Object[] object = getCommand(command);
        if(object[0] == null || ((SimpleCommand)object[0]).getExecuterType() == ExecuterType.CONSOLE) return false;
        try{
